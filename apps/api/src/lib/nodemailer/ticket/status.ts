  ...
  if (email) {
    ...
    await transport
      .sendMail({
        from: email?.reply,
        to: ticket.email,
        subject: `Case #${ticket.Number} status is now ${ticket.isComplete ? "COMPLETED" : "OUTSTANDING"
          }`,
        text: `Hello there, Case #${ticket.Number}, now has a status of ${ticket.isComplete ? "COMPLETED" : "OUTSTANDING"
          }`,
        html: htmlToSend,
      })
      .then((info: any) => {
